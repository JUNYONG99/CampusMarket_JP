...
function fileUploadCheck(value) {
  ...
  $.each(target[0].files, function (index, file) {
    ...
    html[img_count] +=
      ...
      '" name="imgs" style="width : 200px">'; //미리보기 이미지
    ...
    if (
      ...
      alert("파일은 (jpg, png, gif, bmp) 형식만 등록 가능합니다."); //resetFile();
      return false;
    }
    filesArr.push(html);
...
function deleteImg() {
  ...
  flist.forEach((el, index) => {
    // 가져온 요소들로 반복문을 통해 인덱스 가져옴
    el.onclick = () => {
      // 해당 인덱스의 요소가 클릭되면
      console.log(fileArray);
      html.splice(index, 1);
...
function chattingCheck(seller, store_index, title) {
  if (loginState == "로그인/회원가입") {
    ...
    if (seller != null) {
      location.replace(
        "campus_chatting_connect.php?seller=" +
          seller +
          "&store_index=" +
          store_index +
          "&title=" +
...
function bannerChange() {
  if (banner) {
    ...
    banner_buttonLeft.addEventListener("click", () => {
      ...
      banner_slide.style.marginLeft = `-${
        banner.clientWidth * banner_currentIndex
    ...
    banner_buttonRight.addEventListener("click", () => {
      banner_currentIndex++;
      banner_currentIndex =
        banner_currentIndex >= bannerImgs.length
          ? bannerImgs.length - 1
          : banner_currentIndex; // index값이 inner의 총 개수보다 많아질 경우 마지막 인덱스값으로 변경
      banner_slide.style.marginLeft = `-${
        banner.clientWidth * banner_currentIndex
      }px`; // index만큼 margin을 주어 옆으로 밀기
      clearInterval(banner_interval); // 기존 동작되던 interval 제거
...
const banner_getInterval = () => {
  if (banner) {
    return setInterval(() => {
      banner_currentIndex++;
      banner_currentIndex =
        banner_currentIndex >= bannerImgs.length ? 0 : banner_currentIndex;
      banner_slide.style.marginLeft = `-${
        banner.clientWidth * banner_currentIndex
      }px`;
    }, 4000);
...
function boardImgChange() {
  if (board) {
    ...
    buttonRight.addEventListener("click", () => {
      currentIndex++;
      currentIndex =
        currentIndex >= boardImgs.length ? boardImgs.length - 1 : currentIndex; // index값이 inner의 총 개수보다 많아질 경우 마지막 인덱스값으로 변경
      slide.style.marginLeft = `-${board.clientWidth * currentIndex}px`; // index만큼 margin을 주어 옆으로 밀기
      clearInterval(interval); // 기존 동작되던 interval 제거
      interval = getInterval(); // 새로운 interval 등록
    });
...
function emailCheck() {
  ...
  if (check == "") {
    ...
    window.open(
      "email_check.php?join_email=" +
        form.join_email.value +
        "@g.shingu.ac.kr&page=join",
      "캠퍼스 마켓",
